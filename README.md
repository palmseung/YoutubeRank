...
##### Test Coverage : 93% classes, 86% lines covered (마지막 Commit 기준)
...
>- 기존의 Service 클래스 외에 QueryService 클래스를  별도로 생성함으로써, Connection 자원 사용을 정체되게 만드는 OSIV(Open Session In View) 설정을 false로 설정해도, Lazy Loading으로 설정된 데이터를 Controllerd와 View에서도 사용 가능하도록 구성하였습니다.
...
> YouTubeRank의 API의 JSON 응답 안에 키와 밸류가 어떤 의미를 가지는지 해석할 수 있는 API 문서(Profile)를 첨부함으로써 Self-Descriptive 조건을 충족합니다. 이 때, 각 어플리케이션의 상태도 하이퍼링크로 함께 전달함으로써 HATEOAS를 지향하도록 API를 구현하였습니다. 아래는 YouTubeRank의 회원가입 요청에 대한 API 응답 예시입니다.  

><img src="static/img/response-example.png"/>
